...
def get_country_from_city_page(city_relative_path):
	...
	if len(constituent_country) > 0:
		...
		return link[0] if len(link) > 0 else '/wiki/' + constituent_country[0].xpath(".//text()")[0].strip().replace(' ', '_')
	country = infobox.xpath("./tbody/tr/th/descendant-or-self::*[contains(text(), 'Country') or contains(text(), 'country')]/ancestor::tr/td")
